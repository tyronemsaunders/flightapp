...
flightsQuery.$inject = ['$resource', '$q', 'flightsQueryForm'];

function flightsQuery($resource, $q, flightsQueryForm) {
	var query = {};
	query.flights;
	...
	function getFlightsFromSkypicker() {
		...
		function prepSkypickerResponse(data, headersGetter, status) {
			...
			//console.log("SkyPickerResponse:" + JSON.stringify(data, null, 4));
			for (var i = 0; i < data.data.length; i++) {
				...
				flight.duration_minutes['return'] = data.data[i].duration['return'];
				flight.duration_minutes.outbound = data.data[i].duration.departure;
